...
class PasswordRepository extends Repository {
    ...
    async create(data) {
        try {
            ...
            const createPasswordsQuery = `INSERT INTO temporaryPasswords (password) VALUES (?)`;
            const [result] = await connection.query(createPasswordsQuery, [data.password]);
            connection.release();
            ...
            console.error('Error creating temporaryPasswords:', error);
            return resultOfRequest(true, 0, 0);
        }
    }
    ...
    async getAll() {
        const pool = await this.pool;
        const connection = await pool.getConnection();
        const createPasswordsQuery = `SELECT * FROM temporaryPasswords`;
    ...
    async getById(id) {
        const pool = await this.pool;
        const connection = await pool.getConnection();
        const getPassworsQuery = `SELECT * FROM temporaryPasswords where id= ?`; 
    ...
    async getPasswordById(id) {
        const pool = await this.pool;
        const connection = await pool.getConnection();
        const getPasswordQuery = `SELECT password FROM temporaryPasswords WHERE id = ?`; 
    ...
    async update(data) {
        const pool = await this.pool;
        const connection = await pool.getConnection();
        const updatePasswordQuery = `UPDATE temporaryPasswords SET ? WHERE id = ?`;
    ...
    async delete(id) {
        const pool = await this.pool;
        const connection = await pool.getConnection();
        const deletePasswordQuery = `DELETE FROM temporaryPasswords where id = ${id}`
